...
# let's create a board to represent the minesweeper game
# thi is so than we can just say "create a new board object", or
...
class Board:
    def __init__(self, dim_size, num_bombs):
        ...
        # initialize a set to keep track of which location we've covered
    ...
    def make_new_board(self):
        ...
        while bombs_planted < self.num_bombs:
            ...
            col = loc % self.dim_size # we want the reminder to tell us what index in that row to look at
    ...
    def assign_values_to_board(self):
        ...
        for r in range(self.dim_size):
            for c in range(self.dim_size):
                if self.board[r][c] == '*':
                    # if this is already a bomb, we don't to calculate anything
                    continue
                self.board[r][c] = self.get_num_neighboring_bombs(r, c)

    def get_num_neighboring_bombs(self, row, col):
        # let's iterate through each of the neigboring positions and sum number of bombs
    ...
    def dig(self, row, col):
        ...
        # dig a location with not neighboring bombs -> recursively dig neigbors!
        ...
        for r in range(max(0, row-1), min(self.dim_size-1, row+1)+1):
            for c in range(max(0, col-1), min(self.dim_size-1, col+1)+1):
                if (r, c) in self.dug:
                    continue # don't dig where you've already dig
                self.dig(r, c)
    ...
    def __str__(self):
        # return a board to the player

        # first, let's create a new array that represents what the user should see
...
def play(dim_size=10, num_bombs=10):
    # step 1: create the board and plant the boms
